...
class Triangle(object):
    ...
    def area(self):
        ...
        #        They include the Example in the above doc-string.
        # ---------------------------------------------------------------------
        leng1 = self.a.distance_from(self.b)
        leng2 = self.b.distance_from(self.c)
        leng3 = self.c.distance_from(self.a)
        s = .5 * (leng1 + leng2 + leng3)
        return round(math.sqrt(s *(s - leng1) * (s - leng2) * (s - leng3)))
    ...
    def return_doubled_triangle(self):
        ...
        #        They include the Example in the above doc-string.
        # -------------------------------------------------------------------------
        ta = Point(self.a.x * 2, self.a.y * 2)
        tb = Point(self.b.x * 2, self.b.y * 2)
        tc = Point(self.c.x * 2, self.c.y * 2)
        newtriangle = Triangle(ta, tb, tc)
        if self.area() > self.biggest:
            self.biggest = self.area()
        self.change = self.change + 1
        return newtriangle
